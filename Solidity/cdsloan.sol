...
contract Loan is usingOraclize {
    ...
    // in the future this function would implment a modifier function
    function Kill() onlyCashprovider{ // working
    ...
    function reportDefault (uint _amount)
    {
        ...
        if(msg.sender!=Cashtaker)
        {
            ...
            else if (lent < counter)
            {
                ...
                userHisory.populateHistory(loanID, amountleft);
               // do something here
               // onwership would change ie the cashprovider would be changed to the cds seller
    ...
    function changeonwership( address cdsseller) external 
    {
        Cashprovider= cdsseller;
        userHisory.changename(cdsseller, loanID);
